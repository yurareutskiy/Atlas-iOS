...
- (void)setUp
{
    ...
    self.testInterface = [[ATLTestInterface alloc] init];
}
...
{
    self.viewController = [ATLSampleConversationListViewController conversationListViewControllerWithLayerClient:(LYRClient *)self.client];
...
- (void)testToVerifyGlobalDeletionButtonFunctionality
{
    ...
    ATLUserMock *mockUser1 = [ATLUserMock userWithMockUserName:ATLMockUserNameKlemen];
    LYRConversationMock *conversation1 = [self.client conversationWithParticipants:[NSSet setWithObject:mockUser1.participantIdentifier] lastMessageText:message1];
    [tester swipeViewWithAccessibilityLabel:[self.testInterface conversationLabelForConversation:conversation1 layerClient:self.client] inDirection:KIFSwipeDirectionLeft];
...
- (void)testToVerifyColorAndFontChangeFunctionality
{
    ...
    NSString *conversationLabel = [self.testInterface conversationLabelForConversation:conversation1 layerClient:self.client];
...
- (void)testToVerifyCustomCellClassNotConformingToProtocolRaisesException
{
    ...
    expect(^{ [weakSelf.viewController setCellClass:[UITableViewCell class]]; }).to.raise(NSInternalInconsistencyException);
}
...
- (void)testToVerifyConversationListViewControllerDataSource
{
    ...
    __block LYRConversation *conversation;
...
- (void)testToVerifyDelegateIsNotifiedOfConversationSelection
{
    ...
    [tester tapViewWithAccessibilityLabel:[self.testInterface conversationLabelForConversation:conversation1 layerClient:self.client]];
    [delegateMock verify];
}
...
- (void)testToVerifyDelegateIsNotifiedOfSearch
{
    ...
    [tester swipeViewWithAccessibilityLabel:[self.testInterface conversationLabelForConversation:conversation1 layerClient:self.client]  inDirection:KIFSwipeDirectionDown];
...
{
    self.viewController = [ATLConversationListViewController conversationListViewControllerWithLayerClient:(LYRClient *)self.client];
    self.viewController.allowsEditing = YES;
...
{
    LYRConversationMock *mockConversation = [self.client newConversationWithParticipants:[NSSet setWithObject:mockUser.participantIdentifier] options:nil error:nil];
    LYRMessageMock *mockMessage = [self.client newMessageWithParts:@[[LYRMessagePart messagePartWithText:lastMessageText]] options:nil error:nil];
    [mockConversation sendMessage:mockMessage error:nil];
    [tester waitForViewWithAccessibilityLabel:[self.testInterface conversationLabelForConversation:mockConversation layerClient:self.client]];
...
- (void)deleteConversation:(LYRConversationMock *)conversation deletionMode:(LYRDeletionMode)deletionMode
{
    switch (deletionMode) {
        case LYRDeletionModeAllParticipants:
            ...
            [tester waitForAbsenceOfViewWithAccessibilityLabel:[self.testInterface conversationLabelForConversation:conversation layerClient:self.client]];
            break;
