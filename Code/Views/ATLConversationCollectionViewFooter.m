...
{
    NSLog(@"%@", recipientStatus);
    NSMutableAttributedString *checkMarks;
    if ([[recipientStatus string] isEqualToString:@"Pending"]) {
        checkMarks = [[NSAttributedString alloc] initWithString:@"" attributes:[recipientStatus attributesAtIndex:0 effectiveRange:nil]];
    } else if ([[recipientStatus string] isEqualToString:@"Sent"]) {
        checkMarks = [[NSAttributedString alloc] initWithString:@"✓" attributes:[recipientStatus attributesAtIndex:0 effectiveRange:nil]];
    } else if ([[recipientStatus string] isEqualToString:@"Delivered"]) {
        checkMarks = [[NSAttributedString alloc] initWithString:@"✓" attributes:[recipientStatus attributesAtIndex:0 effectiveRange:nil]];
    } else if ([[recipientStatus string] isEqualToString:@"Read"]) {
        checkMarks = [[NSAttributedString alloc] initWithString:@"✓✓" attributes:[recipientStatus attributesAtIndex:0 effectiveRange:nil]];
    }
    self.recipientStatusLabel.attributedText = checkMarks;
}
